function flatEntries(){
  return sessions.flatMap(s => (s.entries || []).map(e => {
    ...
    const m = created.match(/[T\s](\d{2}):(\d{2})/);
...
function flatExpensesFixed(){
  return flatExpenses().map(e => {
    const created = String(e.created_at || '');
    const m = created.match(/[T\s](\d{2}):(\d{2})/);
    return {...e, date: created.slice(0,10), hhmm: m ? (m[1] + ':' + m[2]) : '00:00'};
  });
}

function buildMetric(es, xs){
...
function metrics(start, end){
  ...
  return buildMetric(
    flatEntries().filter(e => inRange(e, rge)),
    flatExpensesFixed().filter(e => inRange(e, rge))
  );
}
...
function metricDateFull(dateObj){
  ...
  return buildMetric(
    flatEntries().filter(e => e.date === key),
    flatExpensesFixed().filter(e => e.date === key)
  );
}
...
function metricDateUntilSameTime(dateObj){
  ...
  return buildMetric(
    flatEntries().filter(e => e.date === key && e.hhmm <= hhmm),
    flatExpensesFixed().filter(e => e.date === key && e.hhmm <= hhmm)
  );
}
...
function renderDayDash(){
  ...
  const closeGrossForecast = m.gross + (baseCount ? (yRemainingGross + pRemainingGross) / baseCount : 0);
  const closeNetForecast = m.net + (baseCount ? (yRemainingNet + pRemainingNet) / baseCount : 0);

  if($('dayScore')) $('dayScore').textContent = statScore(m);
  if($('daySummaryText')) $('daySummaryText').textContent = 'Hoje comparado com ontem no mesmo horário e com a ' + wd + ' passada. A busca de ontem agora é por data real do lançamento.';
  ...
  const yBest = topHours(byHour(yFull), 6);
  const refBest = topHours(byHour(pFull), 6);
  const merged = [...yBest.map(h => ({...h, src:'ontem'})), ...refBest.map(h => ({...h, src:wd+' passada'}))].sort((a,b)=>b.gross-a.gross).slice(0,8);

  drawBarSeries('dayHourlyChart', byHour(yFull).map(x=>hourLabel(x.hour)), byHour(yFull).map(x=>x.gross), 'Ontem por horário');
...
function renderWeekDash(){
  ...
  if($('weekSummaryText')) $('weekSummaryText').textContent = 'Semana atual comparada com a semana passada. Entradas fora da carteira entram no bruto, KM, lucro e relatórios; só não alteram a carteira.';

  if($('weekCards')) $('weekCards').innerHTML =
  ...
  const linhasDias = diasAtual.map((d, i) => {
    ...
    const detalhes = 'diferença ' + signedMoney(dif) + '<br>líquido: ' + brl(d.net) + ' vs ' + brl(p.net) + '<br>corridas: ' + d.rides + ' vs ' + p.rides + '<br>KM: ' + km(d.km) + ' vs ' + km(p.km) + '<br>horas ativas: ' + horasTxt(d.activeHours) + ' vs ' + horasTxt(p.activeHours) + '<br>R$/h ativa: ' + brl(porHora(d)) + ' vs ' + brl(porHora(p)) + '<br>R$/km líquido: ' + brl(d.netPerKm) + ' vs ' + brl(p.netPerKm) + '<br>fora da carteira: ' + brl(d.outWalletGross || 0);
    return rowItem(nomes[i] + ' • atual ' + brl(d.gross) + ' / passada ' + brl(p.gross), detalhes, dif >= 0 ? 'good' : 'bad');
  }).join('');

  renderList('weekBestDays', '<h3 class="hint">Dia por dia: semana atual x semana passada</h3>' + linhasDias, 'Sem dados para comparar por dia.');

  const horasAtual = topHours(byHour(atual), 8);
  const horasPassada = topHours(byHour(passadaCheia), 8);
  renderList('weekBestHours', '<h3 class="hint">Horários fortes da semana atual</h3>' + (horasAtual.length ? horasAtual.map(h => rowItem(hourLabel(h.hour) + ' • ' + brl(h.gross), h.rides + ' corrida(s) • ' + km(h.km) + ' • líquido ' + brl(h.net), 'info')).join('') : '<p class="hint">Sem horário forte nesta semana.</p>') + '<h3 class="hint">Horários fortes da semana passada</h3>' + (horasPassada.length ? horasPassada.map(h => rowItem(hourLabel(h.hour) + ' • ' + brl(h.gross), h.rides + ' corrida(s) • ' + km(h.km) + ' • líquido ' + brl(h.net), 'purple')).join('') : '<p class="hint">Semana passada sem corridas registradas.</p>'), 'Sem horários fortes.');

  const avisoSemanaPassada = passadaCheia.gross > 0 ? 'Semana passada inteira fechou em ' + brl(passadaCheia.gross) + ' bruto, ' + brl(passadaCheia.net) + ' líquido, ' + km(passadaCheia.km) + ', ' + horasTxt(passadaCheia.activeHours) + ' ativas e ' + passadaCheia.rides + ' corrida(s).' : 'Semana passada inteira está zerada no sistema. Isso significa que não há corridas registradas naquele período ou que o período anterior não teve lançamentos.';

  renderList('weekInsights', progressLine('Bruto semana atual x passada até agora', atual.gross, passadaAteAgora.gross) + progressLine('Bruto semana atual x passada inteira', atual.gross, passadaCheia.gross) + progressLine('Líquido semana atual x passada inteira', atual.net, passadaCheia.net) + progressLine('Corridas semana atual x passada inteira', atual.rides, passadaCheia.rides, 'num') + progressLine('KM semana atual x passada inteira', atual.km, passadaCheia.km, 'km') + rowItem('Entradas fora da carteira', brl(atual.outWalletGross) + ' aparecem nos dashboards e relatórios, mas não alteram a carteira.', atual.outWalletGross > 0 ? 'warning' : 'info') + rowItem('Horas ativas da semana', horasTxt(atual.activeHours) + ' atual vs ' + horasTxt(passadaCheia.activeHours) + ' semana passada inteira • R$/h atual ' + brl(atual.perActiveHour) + ' vs ' + brl(passadaCheia.perActiveHour), changeClass(atual.perActiveHour, passadaCheia.perActiveHour)) + rowItem('Resumo da semana passada', avisoSemanaPassada, passadaCheia.gross > 0 ? 'info' : 'warning'));
}
...
setTimeout(function(){ if(typeof renderDashboard === 'function') renderDashboard(); }, 3500);
